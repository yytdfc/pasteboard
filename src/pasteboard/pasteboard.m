...
static PyObject *
pasteboard_new(PyTypeObject *type,
...
pasteboard_get_contents(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "|O!p", kwlist, &PasteboardTypeType, &type, &diff)) {
        return NULL;
    }
...
static PyObject *
set_contents(NSPasteboard *board,
             PasteboardTypeState *type,
             const char *bytes,
             Py_ssize_t length)
{
    switch (type->read) {
    ...
    case STRING: {
        NSString *str = [[NSString alloc] initWithBytesNoCopy:(void *)bytes
                                                       length:length
...
pasteboard_get_file_urls(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    NSArray<NSURL *> *files = [state->board readObjectsForClasses:classes
                                                          options:options];
    ...
    if (len != pos) {
        if (_PyTuple_Resize(&urls, pos) != 0) {
            PyErr_SetString(PyExc_RuntimeError,
                            "Internal error: failed to resize tuple");
            return NULL;
        }
    }
    return urls;
